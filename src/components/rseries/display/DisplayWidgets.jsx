...
/** SpO₂ % — cyan numeric (region paramSpO2). */
export function Spo2Widget({ value = '- - -' } = {}) {
  return (
    <g fontFamily={T}>
      <text x="12" y="66" fill={phosphor.cyan} fontSize="30" fontWeight="700">{value}</text>
    </g>
  )
}

/** NIBP sys/dia (mean) mmHg + time of reading — white (region paramNIBP, local). */
export function NibpWidget({ sys, dia, mean, time } = {}) {
  const has = sys != null && dia != null
  return (
    <g fontFamily={T} fill={phosphor.white}>
      <text x="12" y="82" fontSize="22" fontWeight="700">{has ? `${sys}/${dia}` : '- - -'}</text>
...
export function Co2Widget({ etco2, rr } = {}) {
  return (
    <g fontFamily={T} fill={phosphor.amber}>
...
export function EcgHrWidget({ lead = 'II', gain = 'x1', hr = '- -' } = {}) {
  return (
    <g fontFamily={T} fill={phosphor.green}>
      <text x="6" y="22" fontSize="13">ECG</text>
      <text x="6" y="42" fontSize="12">{lead}</text>
      <text x="34" y="42" fontSize="12">{gain}</text>
      <text x="72" y="24" fontSize="14">♥</text>
      <text x="212" y="72" textAnchor="end" fontSize="46" fontWeight="700">{hr}</text>
    </g>
  )
}
 ...
 */
export function waveTraces({ rhythm = 'Normal Sinus', pads = false, filtered, lead, pleth = true, capno = true } = {}) {
  const t = [{ label: pads ? 'PADS' : lead || 'ECG', base: B.ecg, color: phosphor.green, d: ecgFor(rhythm), height: 78 }]
  if (pads) t.push({ label: 'FIL', base: B.pleth, color: phosphor.green, d: ecgFor(filtered || rhythm), height: 70 })
...
export function WaveformFieldWidget({ traces = [] } = {}) {
  return (
    <g>
      {traces.map((tr, i) => (
        <g key={i}>
 ...
 *   spo2, nibp:{sys,dia,mean,time}, co2:{etco2,rr}
 *   mode, hr, lead, gain
 *   cpr:{release,perfusion,active}
 *   waveform:{rhythm,pads,filtered,lead,pleth,capno}
 *   message:{text,tone}, readout:{time,center,right}, softkeys:[…]
 *
...
export function mountWidgets(model = {}) {
  const s = {}
  if (model.spo2 != null) s.paramSpO2 = <Spo2Widget value={model.spo2} />
  if (model.nibp) s.paramNIBP = <NibpWidget {...model.nibp} />
  if (model.co2) s.paramCO2RR = <Co2Widget {...model.co2} />
  if (model.mode != null) s.statusClockMode = <ModeWidget mode={model.mode} />
  if (model.cpr) s.statusCpr = <CprWidget {...model.cpr} />
  if (model.hr != null || model.lead != null) s.statusEcgHr = <EcgHrWidget hr={model.hr} lead={model.lead} gain={model.gain} />
  if (model.waveform) s.waveformField = <WaveformFieldWidget traces={waveTraces(model.waveform)} />
